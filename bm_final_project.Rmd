...
```{r}
multi.fit <- lm(TARGET_deathRate ~ ., data = cancer)
summary(multi.fit)
```
...
cancer_model = cancer %>% 
  select(TARGET_deathRate, avgAnnCount, incidenceRate, AvgHouseholdSize, studyPerCap, income_cutoff, age_cutoff, PercentMarried, PctPrivateCoverage, PctWhite)  
